...
(function(devices) {
	...
	devices.handleUrl = function(handler, pathname, request, response) {
		...
		if (devices.pendingRequests[nonce] != undefined) {
			console.log("Requests are pending");
		...
		//This function is responsible for handing the data that is sent over TCP.
		request.on('data', function(data) {
			...
			var vals = [];
			if (buf.length > 0) {
				// for(var ii = 0; ii < buf.length-1; ii += 2) {
					// var unsigned = buf.readUInt16LE(ii);
					// //console.log('Data: ' + unsigned + '      Count: ' + count);
					// vals.push(unsigned);
				// }
					//console.log('Values: ' + buf.length/2);
					//devices.callDataHandlers('deviceData', deviceState, buf);
				for (var xx = 0; xx<buf.length-1; xx += 2) {
					vals.push(buf.readUInt16LE(xx) + '\n');
				}
				fs.appendFile('data.txt', vals, 'utf8', (err) => {
						if (err) throw err;
